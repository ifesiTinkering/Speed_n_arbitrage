...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print(json.dumps({"status": "error", "message": "Usage: python3 execute_transaction.py <maxFeePerGas (Gwei)> <maxPriorityFeePerGas (Gwei)> <gas>"}))
        sys.exit(1)

    try:
        maxFeePerGas = Web3.to_wei(float(sys.argv[1]), 'gwei')
        maxPriorityFeePerGas = Web3.to_wei(float(sys.argv[2]), 'gwei')
        gas = int(sys.argv[3])

        execute_transaction(maxFeePerGas, maxPriorityFeePerGas, gas)
        print(json.dumps({"status": "success", "message": "Transaction executed successfully"}))
